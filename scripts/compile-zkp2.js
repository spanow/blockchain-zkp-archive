...
async function main() {
    try {
        ...
        // Étape 2: Vérifier si le fichier ptau existe
        const ptauPath = path.join('build', 'circuits', 'pot12_final.ptau');
        if (!fs.existsSync(ptauPath)) {
            console.log('\nÉtape 2: Le fichier ptau n\'existe pas');
            console.log('Veuillez d\'abord exécuter:');
            console.log('node scripts/generate-ptau.js');
            console.log('Puis relancez ce script.');
            return;
        }
        
        console.log('\nÉtape 3: Génération des clés spécifiques au circuit');
        // Utiliser snarkjs en ligne de commande avec notre fichier ptau local
        execSync('npx snarkjs groth16 setup build/circuits/FileAccessVerifier.r1cs build/circuits/pot12_final.ptau build/circuits/FileAccessVerifier_0000.zkey -v', { stdio: 'inherit' });
